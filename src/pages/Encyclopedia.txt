...
import { ArrowLeft, Atom, Calculator, Cog, Users, BookOpen, Sparkles, Brain, Scale } from "lucide-react";
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {categorias.map((categoria) => (
              <div
                key={categoria.title}
                className="group p-6 bg-card rounded-2xl shadow-soft hover:shadow-elevated transition-all duration-500 hover:-translate-y-2 cursor-pointer"
              >
                <div className={`w-14 h-14 bg-gradient-to-br ${categoria.color} rounded-xl flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300`}>
                  <categoria.icon className="w-7 h-7 text-white" />
                </div>
